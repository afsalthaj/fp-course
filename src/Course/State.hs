...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s b
  (=<<) fsab sa =
    State(\s -> (let (a, s') = runState sa s in (runState (fsab a) s')))
...
distinct ::
  ...
  -> List a
distinct l =
    eval (filtering (\a -> State(\s -> (S.notMember a s, S.insert a s))) l) S.empty
