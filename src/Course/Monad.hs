...
-- >>> ((*) =<< (+10)) 7
-- 119
-- ap   of reader is (t -> (a -> b)) -> (t a) -> t b  (in Scala, f[X => (A => B)] and f[X => A] returns f[X => B])
-- Bind of reader is (a -> (t -> b)) -> (t a ) -> t b (in Scala, A => f[X => B]   and f[X => A] returns f[X => B])
